package ba.etfrma.bookish.provider
...
class BookContentProvider : ContentProvider() {
    ...
    companion object {
        const val AUTHORITY = "ba.etfrma.bookish.provider"
        ...
        when (uriMatcher.match(uri)) {
            BOOKS_ID -> {
                ...
                val updatedBook = book.copy(
                    title = values?.getAsString("title") ?: book.title,
                    publisher = values?.getAsString("publisher") ?: book.publisher,
                    infoLink = values?.getAsString("infoLink") ?: book.infoLink,
                    description = values?.getAsString("description") ?: book.description,
                    thumbnail = values?.getAsString("thumbnail") ?: book.thumbnail
                )
    ...
    // Pretvori ContentValues u FullBook model
